...
public class SoundWaveGenerator extends Thread {
    ...
    private WavePlayer variableWavePlayers[];
...
decimalWavePlayers = new WavePlayer[10];
        variableWavePlayers = new WavePlayer[GameInformationFrequencies.FREQUENCIES.length-1];

        for(int a =0;a<10;a++)
            decimalWavePlayers[a] =  new WavePlayer(audioContext, Variables.DECIMAL_FREQUENCIES[a], Buffer.SINE);

        for(int a =0;a<variableWavePlayers.length-1;a++)
            variableWavePlayers[a] =  new WavePlayer(audioContext, GameInformationFrequencies.FREQUENCIES[a], Buffer.SINE);
    ...
    public void run() {
        ...
        for(int a =0;a<variableWavePlayers.length-1;a++)
            if(frequency == (GameInformationFrequencies.FREQUENCIES[a]))
                audioContext.out.addInput(variableWavePlayers[a]);
